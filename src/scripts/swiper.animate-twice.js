...
function swiperAnimate(a) {
    ...
    for (i = 0; i < b.length; i++) {
        ...
        //动画样式
         var  effect = b[i].hasAttribute("in_from") ? b[i].attributes["in_from"].value : "";
         b[i].className = b[i].className + "  " + effect + " " + "animated";
         var style = b[i].attributes["style"].value;
        //动画持续时间
         var  duration = b[i].hasAttribute("in_during") ?parseInt(b[i].attributes["in_during"].value) : "";
         duration && (style = style + "animation-duration:" + (duration/1000+'s') + ";-webkit-animation-duration:" + (duration/1000+'s') + ";");
        //动画延迟时间（单位秒）
         var delay = b[i].hasAttribute("in_at") ? parseInt(b[i].attributes["in_at"].value) : "";
         delay && (style = style + "animation-delay:" + (delay/1000+'s') + ";-webkit-animation-delay:" + (delay/1000+'s') + ";");

        //切换效果速度
        var timing = b[i].hasAttribute("in_use") ? b[i].attributes["in_use"].value : "";
...
function executionAnimate() {
    ...
    if(nwoOutAni){
        for(var i in nwoOutAni.element) {
            ...
            var effect = em.hasAttribute("out_to") ? em.attributes["out_to"].value : "";
            em.className = em.className + "  " + effect + " ";
            var style = em.attributes["style"].value;
            //动画持续时间
            var duration = em.hasAttribute("out_during") ? parseInt(em.attributes["out_during"].value) : "";
            duration && (style = style + "animation-duration:" + (duration/1000+'s') + ";-webkit-animation-duration:" + (duration/1000+'s') + ";");
            //动画延迟时间（单位秒）
            var delay = em.hasAttribute("out_at") ? parseInt(em.attributes["out_at"].value) : "";
            delay && (style = style + "animation-delay:" + (delay/1000+'s') + ";-webkit-animation-delay:" + (delay/1000+'s') + ";");
            //切换效果速度
            var timing = em.hasAttribute("out_use") ? em.attributes["out_use"].value : "";
            timing && (style = style + "transition-timing-function:" + timing + "; -webkit-transition-timing-function:" + timing + ";");
...
function clearSwiperAnimate() {
    ...
    for (allBoxes = window.document.documentElement.querySelectorAll(".ani"), i = 0; i < allBoxes.length; i++) {
        allBoxes[i].hasAttribute("swiper-animate-style-cache") && allBoxes[i].setAttribute("style", allBoxes[i].attributes["swiper-animate-style-cache"].value);
        allBoxes[i].style.visibility = "hidden";
        allBoxes[i].className = allBoxes[i].className.replace("animated","");
        allBoxes[i].hasAttribute("in_from") && (allBoxes[i].className = allBoxes[i].className.replace(allBoxes[i].attributes["in_from"].value,""));
        allBoxes[i].hasAttribute("out_to") && (allBoxes[i].className = allBoxes[i].className.replace(allBoxes[i].attributes["out_to"].value,""));
    }
}
